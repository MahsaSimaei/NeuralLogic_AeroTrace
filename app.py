...
def save_uploaded_file(uploaded_file, suffix: str = "") -> Optional[str]:
    ...
    try:
        ...
        if file_size_mb > 500:
            st.error(f"❌ Archivo demasiado grande: {file_size_mb:.1f} MB (máximo: 500 MB)")
            return None
...
def main():
    ...
    with st.sidebar:
        ...
        CONFIG.LINE_START = sv.Point(0, line_pos_y)
        CONFIG.LINE_END = sv.Point(1920, line_pos_y)
        
        # Límite de frames
        st.subheader("🎞️ Procesamiento")
        max_frames_option = st.selectbox(
            "Frames a procesar",
            ["Todos", "500", "1000", "1500", "3000"],
            index=0,
        ...
        st.info("""
        💡 **Consejos:**
        - Usa ZIP de imágenes para mejor control
        - Baja la confianza (0.20) para más detecciones
        - Ajusta la línea según tu escena
        """)
                ...
                try:
                    ...
                    if image_files:
                        ...
                        if first_img is not None:
                            ...
                            st.image(first_img_rgb, caption="Preview: Primera imagen", use_container_width=True)
    ...
    if input_path:
        ...
        if run_button:
            ...
            try:
                ...
                video_out, df_metrics, incidents = system.run(
